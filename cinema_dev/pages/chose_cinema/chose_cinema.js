...
Page({
    ...
    loadData() {
        ...
        params.day = this.data.queryDays[this.data.currentDay]
        params.filmUid = this.data.filmUid
        getHasScheduleCinemaList(params).then(res=>{
            ...
            const c = this.data.cinemaList.length + res.data.data.length
            this.data.listQuery.offset = c
            this.setData({
    ...
    calcHeight() {
        ...
        query.select("#chose_days").boundingClientRect()
        query.select("#tips").boundingClientRect()
        query.exec(res => {
            console.log(res)
            const height = res[0].height + res[1].height + res[2].height + res[3].height
            this.setData({
                height: wx.getSystemInfoSync().windowHeight-height,
                top_layout_height: res[0].height + res[1].height
            })
        })
    },
    loadRegionData() {
    ...
    calcDays(){
        ...
        const days = []
        const queryDays = []
        days.push('今天 '+formatMonthDay(day))
        queryDays.push(formatDate(day))
        for(let i=0;i<6;i++){
            day.setDate(day.getDate()+1)
            if(i<2){
                days.push((i===0?'明天':'后天')+' '+formatMonthDay(day))
            }else{
                days.push(week[day.getDay()]+' '+formatMonthDay(day))
            }
            queryDays.push(formatDate(day))
        }
        this.data.queryDays = queryDays
        this.setData({
            showDays: days,
            queryDays: queryDays
        })
    ...
    goTicket(e){
        ...
        wx.navigateTo({
          url: '/pages/chose_ticket/chose_ticket?cinemaUid='+cinema+'&filmUid='+this.data.filmUid,
        })
    }
})